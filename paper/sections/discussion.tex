...
correct regardless of the masking mechanism (\Cref{prop:hazard-robust}).
...
\begin{enumerate}
    ...
    \item \textbf{Report sensitivity bounds.} When conclusions are sensitive to
        ...
        $\mathbf{P}$ family as sensitivity bounds. This parallels the partial
        identification approach~\citep{manski2003partial}, where the data
        identify a set of possible parameter values rather than a point
        estimate, and reporting the identified set is more honest than
...
\begin{itemize}
    ...
    \item \textbf{Variance estimation.} The coverage metric uses Wald
        intervals based on the model-based Fisher information, which assumes
        correct specification. Under misspecification, the sandwich
        estimator~\citep{huber1967behavior} is more appropriate for variance
        estimation. The observed coverage degradation thus conflates two
        effects: bias moving the point estimate away from the true value, and
        the model-based standard error being incorrect. Disentangling these
        effects via sandwich-based coverage is an avenue for future work.
\end{itemize}
