...
$\hat{\mathbf{k}} = (1.26, 1.17, 1.13)$ and scales
$\hat{\vlambda} = (1000, 897, 847)$. The system hazard at the median failure
...
$s$ & $\hat{k}_1$ & $\hat{k}_2$ & $\hat{k}_3$ &
      ...
      $\hat{h}_T(t_0)$ \\
\midrule
0.0 & 1.26 & 1.17 & 1.13 & 1000 & 897 & 847 & 0.00307 \\
...
with severity. The scale parameter $\hat\lambda_3$ decreases from 847 to 780
(an 8\% shift) as severity increases, while $\hat\lambda_2$ increases from 897
to 945 (a 5\% shift). Shape parameters show smaller absolute changes but
...
($\hat\lambda_3 = 847$, smallest scale), but by $s = 1.0$ component~3 has
